...
appdir=/tmp/EndlessWalls.AppDir
...
cp ../../graphics/main.png $appdir/EndlessWalls.png
pushd $appdir
ln -s EndlessWalls.png .DirIcon
popd

cp EndlessWalls.desktop $appdir
mkdir $appdir/usr
...
cp ../../bin/EndlessWallsFPC $appdir/usr/bin/EndlessWalls
cp -r ../../data $appdir/usr

export ARCH=x86_64

echo en > $appdir/usr/data/texts/deflang
appimagetool-x86_64.AppImage $appdir /tmp/FindingPony-EN-$VERSION-x86_64.AppImage

echo ru > $appdir/usr/data/texts/deflang
appimagetool-x86_64.AppImage $appdir /tmp/FindingPony-RU-$VERSION-x86_64.AppImage
