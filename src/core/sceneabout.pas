...
var str:string ;
begin
  textTitle:=createText(TCommonData.Font,TCommonData.texts.getText('ABOUT_TITLE'),28,SfmlBlack) ;
  ...
  if FileExists('texts'+PATH_SEP+'version.txt') then begin
    ...
    str:='' ;
  textVer:=createText(TCommonData.Font,str,20,SfmlColorFromRGB(40,40,40)) ;

  textInfo:=createText(TCommonData.Font,TCommonData.texts.getText('ABOUT_INFO'),24,SfmlBlack) ;
  textCredits:=createText(TCommonData.Font,TCommonData.texts.getText('ABOUT_CREDITS'),28,SfmlBlack) ;

  if FileExists('texts'+PATH_SEP+'credits.txt') then
    str:=readAllText('texts'+PATH_SEP+'credits.txt').Replace(chr(13),'')
  else
    str:='' ;
  textList:=createText(TCommonData.Font,str,24,SfmlBlack) ;
  Result:=True ;
end ;
